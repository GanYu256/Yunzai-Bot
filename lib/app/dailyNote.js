...
export async function bingCookie (e) {
  ...
  let param = cookie.match(/ltoken=([^;]+;)|ltuid=(\w{0,9})|cookie_token=([^;]+;)|cookie_token_v2=([^;]+;)|login_ticket=([^;]+;)/g);
  ...
  let ltoken,cookie_token
  if (token["cookie_token"]||token["cookie_token_v2"]){
	  	 ltoken = `ltoken=${token["ltoken"]}ltuid=${token["ltuid"]};`;
	 	 cookie_token = `cookie_token=${token["cookie_token"]||token["cookie_token_v2"]} account_id=${token["ltuid"]};`;
	 	 ltoken += cookie_token;
  }
  if (token["cookie_token_v2"]&&(token["account_mid_v2"]||token["ltmid_v2)"])){
		  ltoken = `ltoken_v2=${token["ltoken_v2"]}ltuid=${token["ltuid"]};`;
		  cookie_token = `cookie_token_v2=${token["cookie_token_v2"]} account_id_v2=${token["ltuid"]}account_mid_v2=${token["account_mid_v2"]||token["ltmid_v2"]};`;
		  ltoken += cookie_token;
  } 
  if(!ltoken){
		 e.reply("绑定失败，代码中必须包含【cookie_token】字样，请重新获取！");
      	 return true;	 
  }
  //Bot.logger.mark("cookie:"+ltoken);
  //cookie_token转换uid
